...
$sql = "SELECT * FROM users WHERE action = 'active'";
...
if (mysqli_num_rows($result) > 0) {
    while($row = mysqli_fetch_assoc($result)) {
        ...
        // Get user's last paid invoice
        $sql2 = "SELECT * FROM payment WHERE userid = '$user_id' AND status = 'paid' ORDER BY id DESC LIMIT 1";
        $result2 = mysqli_query($conn, $sql2);

        if (mysqli_num_rows($result2) > 0) {
            $row2 = mysqli_fetch_assoc($result2);
            $last_paid_date = $row2['paid_date'];
        } else {
            // If user has no paid invoices, set last paid date to registration date
            $last_paid_date = $row['reg_date'];
        }
        
        // Check if 25 days have passed since last paid date
        $current_date = date('jS F, Y');
        $next_due_date = date('jS F, Y', strtotime($last_paid_date . '+30 days'));
        
        if ($current_date == $next_due_date) {
            $sql3 = "UPDATE users SET status='inactive',action='inactive' WHERE userid = '$user_id'";
